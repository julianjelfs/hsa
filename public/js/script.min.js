...
(function(window, undefined) {
    ...
    function cloneCopyEvent(src, dest) {
        ...
        if (1 === dest.nodeType) {
            if (data_priv.hasData(src) && (pdataOld = data_priv.access(src), pdataCur = jQuery.extend({}, pdataOld), 
            events = pdataOld.events, data_priv.set(dest, pdataCur), events)) {
    ...
    function defaultPrefilter(elem, props, opts) {
        ...
        for (index in props) if (value = props[index], rfxtypes.exec(value)) {
            if (delete props[index], toggle = toggle || "toggle" === value, value === (hidden ? "hide" : "show")) {
                if ("show" !== value || dataShow === undefined || dataShow[index] === undefined) continue;
                hidden = !0;
            }
            handled.push(index);
        }
        if (length = handled.length) {
            ...
            "hidden" in dataShow && (hidden = dataShow.hidden), toggle && (dataShow.hidden = !hidden), 
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        function boolHandler(elem, name, isXML) {
            var val;
            return isXML ? undefined : (val = elem.getAttributeNode(name)) && val.specified ? val.value : elem[name] === !0 ? name.toLowerCase() : null;
        }
        function interpolationHandler(elem, name, isXML) {
            var val;
            return isXML ? undefined : val = elem.getAttribute(name, "type" === name.toLowerCase() ? 1 : 2);
        }
        ...
        }, rnative = /^[^{]+\{\s*\[native \w/, rquickExpr = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/, rinputs = /^(?:input|select|textarea|button)$/i, rheader = /^h\d$/i, rescape = /'|\\/g, runescape = /\\([\da-fA-F]{1,6}[\x20\t\r\n\f]?|.)/g, funescape = function(_, escaped) {
        ...
        isXML = Sizzle.isXML = function(elem) {
            ...
            docElem = doc.documentElement, documentIsHTML = !isXML(doc), support.getElementsByTagName = assert(function(div) {
        ...
        compile = Sizzle.compile = function(selector, group) {
            ...
            return cached;
        }, Expr.pseudos.nth = Expr.pseudos.eq, setFilters.prototype = Expr.filters = Expr.pseudos, 
        Expr.setFilters = new setFilters(), support.sortStable = expando.split("").sort(sortOrder).join("") === expando, 
        setDocument(), [ 0, 0 ].sort(sortOrder), support.detectDuplicates = hasDuplicate, 
        assert(function(div) {
            if (div.innerHTML = "<a href='#'></a>", "#" !== div.firstChild.getAttribute("href")) for (var attrs = "type|href|height|width".split("|"), i = attrs.length; i--; ) Expr.attrHandle[attrs[i]] = interpolationHandler;
        }), assert(function(div) {
            if (null != div.getAttribute("disabled")) for (var attrs = booleans.split("|"), i = attrs.length; i--; ) Expr.attrHandle[attrs[i]] = boolHandler;
    ...
    Data.uid = 1, Data.accepts = function(owner) {
        ...
        set: function(owner, data, value) {
            ...
            if ("string" == typeof data) cache[data] = value; else if (jQuery.isEmptyObject(cache)) this.cache[unlock] = data; else for (prop in data) cache[prop] = data[prop];
        },
        ...
        access: function(owner, key, value) {
            return key === undefined || key && "string" == typeof key && value === undefined ? this.get(owner, key) : (this.set(owner, key, value), 
        ...
        remove: function(owner, key) {
            ...
            if (key === undefined) this.cache[unlock] = {}; else {
                jQuery.isArray(key) ? name = key.concat(key.map(jQuery.camelCase)) : key in cache ? name = [ key ] : (name = jQuery.camelCase(key), 
                name = name in cache ? [ name ] : name.match(core_rnotwhite) || []), i = name.length;
        ...
        data: function(key, value) {
            ...
            if (key === undefined) {
                if (this.length && (data = data_user.get(elem), 1 === elem.nodeType && !data_priv.get(elem, "hasDataAttrs"))) {
                    for (attrs = elem.attributes; i < attrs.length; i++) name = attrs[i].name, 0 === name.indexOf("data-") && (name = jQuery.camelCase(name.substring(5)), 
    ...
    jQuery.fn.extend({
        ...
        toggleClass: function(value, stateVal) {
            ...
            return jQuery.isFunction(value) ? this.each(function(i) {
                ...
                self[state ? "addClass" : "removeClass"](className); else (type === core_strundefined || "boolean" === type) && (this.className && data_priv.set(this, "__className__", this.className), 
        ...
        removeAttr: function(elem, value) {
            ...
            jQuery.expr.match.boolean.test(name) && (elem[propName] = !1), elem.removeAttribute(name);
        },
        attrHooks: {
            type: {
                set: function(elem, value) {
    ...
    jQuery.event = {
        ...
        fix: function(event) {
            ...
            return 3 === event.target.nodeType && (event.target = event.target.parentNode), 
    ...
    var isSimple = /^.[^:#\[\.,]*$/, rneedsContext = jQuery.expr.match.needsContext, guaranteedUnique = {
        children: !0,
        contents: !0,
        next: !0,
        prev: !0
    ...
    jQuery.fn.extend({
        find: function(selector) {
            ...
            if ("string" != typeof selector) return self = this, this.pushStack(jQuery(selector).filter(function() {
                for (i = 0; l > i; i++) if (jQuery.contains(self[i], this)) return !0;
            }));
            for (matched = [], i = 0; l > i; i++) jQuery.find(selector, this[i], matched);
            return matched = this.pushStack(l > 1 ? jQuery.unique(matched) : matched), matched.selector = (this.selector ? this.selector + " " : "") + selector, 
            matched;
        },
        has: function(target) {
        ...
        jQuery.fn[name] = function(until, selector) {
            ...
            this.length > 1 && (guaranteedUnique[name] || jQuery.unique(matched), "p" === name[0] && matched.reverse()), 
    ...
    wrapMap.optgroup = wrapMap.option, wrapMap.tbody = wrapMap.tfoot = wrapMap.colgroup = wrapMap.caption = wrapMap.col = wrapMap.thead, 
    wrapMap.th = wrapMap.td, jQuery.fn.extend({
        ...
        cleanData: function(elems) {
            for (var data, elem, type, l = elems.length, i = 0, special = jQuery.event.special; l > i; i++) {
                if (elem = elems[i], jQuery.acceptData(elem) && (data = data_priv.access(elem))) for (type in data.events) special[type] ? jQuery.event.remove(elem, type) : jQuery.removeEvent(elem, type, data.handle);
                data_user.discard(elem), data_priv.discard(elem);
            }
        },
    ...
    ajaxLocParts = rurl.exec(ajaxLocation.toLowerCase()) || [], jQuery.fn.load = function(url, params, callback) {
        ...
        ajax: function(url, options) {
            function done(status, nativeStatusText, responses, headers) {
                ...
                modified && (jQuery.etag[cacheURL] = modified)), 204 === status ? statusText = "nocontent" : 304 === status ? statusText = "notmodified" : (statusText = response.state, 
    ...
    jQuery.Animation = jQuery.extend(Animation, {
        ...
        return this.length;
    }, jQuery.fn.andSelf = jQuery.fn.addBack, "object" == typeof module && "object" == typeof module.exports ? module.exports = jQuery : "function" == typeof define && define.amd && define("jquery", [], function() {
        return jQuery;
            ...
            function cacheFactory(cacheId, options) {
                ...
                return caches[cacheId] = {
                    put: function(key, value) {
                        ...
                        refresh(lruEntry), isUndefined(value) || (key in data || size++, data[key] = value, 
        ...
        this.directive = function registerDirective(name, directiveFactory) {
            ...
            function collectDirectives(node, directives, attrs, maxPriority) {
                ...
                switch (nodeType) {
                  case 1:
                    ...
                    for (var attr, name, nName, value, nAttrs = node.attributes, j = 0, jj = nAttrs && nAttrs.length; jj > j; j++) attr = nAttrs[j], 
                    attr.specified && (name = attr.name, nName = directiveNormalize(name.toLowerCase()), 
            ...
            function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection) {
                ...
                $template = undefined, !(terminalPriority > directive.priority)); i++) {
                    ...
                    $compileNode.html(""), childTranscludeFn = compile($template, transcludeFn))), directiveValue = directive.template) if (assertNoDuplicate("template", templateDirective, directive, $compileNode), 
                    templateDirective = directive, directiveValue = denormalizeTemplate(directiveValue), 
                    directive.replace) {
            ...
            function compileTemplateUrl(directives, beforeTemplateNodeLinkFn, $compileNode, tAttrs, $rootElement, replace, childTranscludeFn) {
                ...
                return $compileNode.html(""), $http.get(origAsyncDirective.templateUrl, {
                    cache: $templateCache
            ...
            function addAttrInterpolateDirective(node, directives, value, name) {
                ...
                interpolateFn && directives.push({
                    ...
                    compile: valueFn(function(scope, element, attr) {
                        ...
                        "class" === name && (interpolateFn = $interpolate(attr[name], !0)), attr[name] = undefined, 
        ...
        this.register = function(name, constructor) {
            ...
            return function(constructor, locals) {
                if (isString(constructor)) {
                    ...
                    constructor = controllers.hasOwnProperty(name) ? controllers[name] : getter(locals.$scope, name, !0) || getter($window, name, !0), 
                    assertArgFn(constructor, name, !0);
                }
    ...
    function matchUrl(url, obj) {
        ...
        return match = {
            ...
            search: match[8],
            hash: match[10]
        }, obj && (obj.$$protocol = match.protocol, obj.$$host = match.host, obj.$$port = match.port), 
        match;
    }
    ...
    function convertToHashbangUrl(url, basePath, hashPrefix) {
        ...
        if (decodeURIComponent(match.path) != basePath || isUndefined(match.hash) || 0 !== match.hash.indexOf(hashPrefix)) {
            ...
            if (0 !== match.path.indexOf(pathPrefix)) throw Error('Invalid url "' + url + '", missing path prefix "' + pathPrefix + '" !');
    ...
    function LocationUrl(url, pathPrefix, appBaseUrl) {
        pathPrefix = pathPrefix || "", this.$$parse = function(newAbsoluteUrl) {
            ...
            if (0 !== match.path.indexOf(pathPrefix)) throw Error('Invalid url "' + newAbsoluteUrl + '", missing path prefix "' + pathPrefix + '" !');
            this.$$path = decodeURIComponent(match.path.substr(pathPrefix.length)), this.$$search = parseKeyValue(match.search), 
            this.$$hash = match.hash && decodeURIComponent(match.hash) || "", this.$$compose();
    ...
    function LocationHashbangUrl(url, hashPrefix, appBaseUrl) {
        ...
        this.$$parse = function(url) {
            ...
            if (match.hash && 0 !== match.hash.indexOf(hashPrefix)) throw Error('Invalid url "' + url + '", missing hash prefix "' + hashPrefix + '" !');
            basePath = match.path + (match.search ? "?" + match.search : ""), match = HASH_MATCH.exec((match.hash || "").substr(hashPrefix.length)), 
            this.$$path = match[1] ? ("/" == match[1].charAt(0) ? "" : "/") + decodeURIComponent(match[1]) : "", 
        ...
        this.hashPrefix = function(prefix) {
            ...
            $location = new LocationHashbangUrl(initUrl, hashPrefix, appBaseUrl)), $rootElement.bind("click", function(event) {
                if (!event.ctrlKey && !event.metaKey && 2 != event.which) {
                    ...
                    absHref && !elm.attr("target") && rewrittenUrl && ($location.$$parse(rewrittenUrl), 
                    $rootScope.$apply(), event.preventDefault(), window.angular["ff-684208-preventDefault"] = !0);
                }
    ...
    function parser(text, json, $filter, csp) {
        ...
        function _assignment() {
            ...
            right = logicalOR(), function(scope, locals) {
        ...
        function arrayDeclaration() {
            var elementFns = [];
            if ("]" != peekToken().text) do elementFns.push(expression()); while (expect(","));
    ...
    function $RouteProvider() {
        ...
        this.when = function(path, route) {
            ...
            function switchRouteMatcher(on, when) {
                when = "^" + when.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&") + "$";
                for (var paramMatch, regex = "", params = [], dst = {}, re = /:(\w+)/g, lastMatchedIndex = 0; null !== (paramMatch = re.exec(when)); ) regex += when.slice(lastMatchedIndex, paramMatch.index), 
                regex += "([^\\/]*)", params.push(paramMatch[1]), lastMatchedIndex = re.lastIndex;
        ...
        this.digestTtl = function(value) {
            ...
            Scope.prototype = {
                $new: function(isolate) {
                    ...
                    child.$$listeners = {}, child.$parent = this, child.$$asyncQueue = [], child.$$watchers = child.$$nextSibling = child.$$childHead = child.$$childTail = null, 
    ...
    function $HttpProvider() {
        ...
        this.$get = [ "$httpBackend", "$browser", "$cacheFactory", "$rootScope", "$q", "$injector", function($httpBackend, $browser, $cacheFactory, $rootScope, $q, $injector) {
            function $http(config) {
                ...
                var promise, reqTransformFn = config.transformRequest || $config.transformRequest, respTransformFn = config.transformResponse || $config.transformResponse, defHeaders = $config.headers, reqHeaders = extend({
                    "X-XSRF-TOKEN": $browser.cookies()["XSRF-TOKEN"]
                }, defHeaders.common, defHeaders[lowercase(config.method)], config.headers), reqData = transformData(config.data, headersGetter(reqHeaders), reqTransformFn);
                return isUndefined(config.data) && delete reqHeaders["Content-Type"], promise = sendReq(config, reqData, reqHeaders), 
                promise = promise.then(transformResponse, transformResponse), forEach(responseInterceptors, function(interceptor) {
            ...
            function sendReq(config, reqData, reqHeaders) {
                ...
                config.cache && "GET" == config.method && (cache = isObject(config.cache) ? config.cache : defaultCache), 
                ...
                return cachedResp || $httpBackend(config.method, url, reqData, done, reqHeaders, config.timeout, config.withCredentials), 
                promise;
            }
            function buildUrl(url, params) {
                ...
                return forEachSorted(params, function(value, key) {
                    null != value && value != undefined && (isObject(value) && (value = toJson(value)), 
            ...
            createShortMethodsWithData("post", "put"), $http.defaults = $config, $http;
        } ];
    }
    ...
    function createHttpBackend($browser, XHR, $browserDefer, callbacks, rawDocument, locationProtocol) {
        ...
        return function(method, url, post, callback, headers, timeout, withCredentials) {
            function completeRequest(callback, status, response, headersString) {
                ...
                status = "file" == protocol ? response ? 200 : 404 : status, status = 1223 == status ? 204 : status, 
            ...
            if ($browser.$$incOutstandingRequestCount(), url = url || $browser.url(), "jsonp" == lowercase(method)) {
                ...
                callbacks[callbackId] = function(data) {
                    ...
                    callbacks[callbackId].data ? completeRequest(callback, 200, callbacks[callbackId].data) : completeRequest(callback, -2), 
    ...
    function limitToFilter() {
        return function(array, limit) {
            ...
            for (limit > array.length ? limit = array.length : limit < -array.length && (limit = -array.length), 
            limit > 0 ? (i = 0, n = limit) : (i = array.length + limit, n = array.length); n > i; i++) out.push(array[i]);
            return out;
        };
    }
    ...
    function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
        ...
        if (pattern && (pattern.match(/^\/(.*)\/$/) ? (pattern = new RegExp(pattern.substr(1, pattern.length - 2)), 
    ...
    var jqLite, jQuery, angularModule, nodeName_, msie = int((/msie (\d+)/.exec(lowercase(navigator.userAgent)) || [])[1]), slice = [].slice, push = [].push, toString = Object.prototype.toString, angular = window.angular || (window.angular = {}), uid = [ "0", "0", "0" ];
    ...
    forEach("input,select,option,textarea,button,form".split(","), function(value) {
        ...
        text: extend(9 > msie ? function(element, value) {
            ...
            return isUndefined(value) ? element.textContent : (element.textContent = value, 
            void 0);
        }, {
            $dv: ""
        }),
        val: function(element, value) {
            return isUndefined(value) ? element.value : (element.value = value, void 0);
        },
    ...
    var ngSubmitDirective = ngDirective(function(scope, element, attrs) {
    ...
    }), ngIncludeDirective = [ "$http", "$templateCache", "$anchorScroll", "$compile", function($http, $templateCache, $anchorScroll, $compile) {
        return {
            restrict: "ECA",
            terminal: !0,
            compile: function(element, attr) {
                ...
                return function(scope, element) {
                    ...
                    scope.$watch(srcExp, function(src) {
                        ...
                        src ? $http.get(src, {
                            ...
                            element.html(response), $compile(element.contents())(childScope), !isDefined(autoScrollExp) || autoScrollExp && !scope.$eval(autoScrollExp) || $anchorScroll(), 
        ...
        compile: function(element, attr, linker) {
            return function(scope, iterStartElement, attr) {
                ...
                if (lhs = match[1], rhs = match[2], match = lhs.match(/^(?:([\$\w]+)|\(([\$\w]+)\s*,\s*([\$\w]+)\))$/), 
                !match) throw Error("'item' in 'item in collection' should be identifier or (key, value) but got '" + lhs + "'.");
                ...
                scope.$watch(function(scope) {
                    ...
                    if (isArray(collection)) array = collection || []; else {
                        array = [];
                        for (key in collection) collection.hasOwnProperty(key) && "$" != key.charAt(0) && array.push(key);
                        array.sort();
                    }
                    for (arrayBound = array.length - 1, index = 0, length = array.length; length > index; index++) key = collection === array ? index : array[index], 
                    value = collection[key], last = lastOrder.shift(value), last ? (childScope = last.scope, 
                    nextOrder.push(value, last), index === last.index ? cursor = last.element : (last.index = index, 
                    cursor.after(last.element), cursor = last.element)) : childScope = scope.$new(), 
                    childScope[valueIdent] = value, keyIdent && (childScope[keyIdent] = key), childScope.$index = index, 
                    childScope.$first = 0 === index, childScope.$last = index === arrayBound, childScope.$middle = !(childScope.$first || childScope.$last), 
                    last || linker(childScope, function(clone) {
                    ...
                    for (key in lastOrder) if (lastOrder.hasOwnProperty(key)) for (array = lastOrder[key]; array.length; ) value = array.pop(), 
                    value.element.remove(), value.scope.$destroy();
                    lastOrder = nextOrder;
                });
            };
        }
        ...
        link: function(scope, element, attr, ctrl) {
            ...
            scope.$watch(watchExpr, function(value) {
                selectedElement && (selectedScope.$destroy(), selectedElement.remove(), selectedElement = selectedScope = null), 
                (selectedTransclude = ctrl.cases["!" + value] || ctrl.cases["?"]) && (scope.$eval(attr.change), 
                selectedScope = scope.$new(), selectedTransclude(selectedScope, function(caseElement) {
    ...
    }), ngViewDirective = [ "$http", "$templateCache", "$route", "$anchorScroll", "$compile", "$controller", function($http, $templateCache, $route, $anchorScroll, $compile, $controller) {
        return {
            restrict: "ECA",
            terminal: !0,
            link: function(scope, element, attr) {
                ...
                function update() {
                    ...
                    if (template) {
                        ...
                        controller = $controller(current.controller, locals), element.children().data("$ngControllerController", controller)), 
            ...
            link: function(scope, element, attr, ctrls) {
                ...
                function Options(scope, selectElement, ctrl) {
                    function render() {
                        ...
                        for (multiple && (selectedSet = new HashMap(modelValue)), index = 0; length = keys.length, 
                        length > index; index++) locals[valueName] = values[keyName ? locals[keyName] = keys[index] : index], 
                        optionGroupName = groupByFn(scope, locals) || "", (optionGroup = optionGroups[optionGroupName]) || (optionGroup = optionGroups[optionGroupName] = [], 
                        optionGroupNames.push(optionGroupName)), multiple ? selected = selectedSet.remove(valueFn(scope, locals)) != undefined : (selected = modelValue === valueFn(scope, locals), 
                        selectedSet = selectedSet || selected), label = displayFn(scope, locals), label = label === undefined ? "" : label, 
                        optionGroup.push({
                    ...
                    nullOption.remove()), selectElement.html(""), selectElement.bind("change", function() {
                        scope.$apply(function() {
                            ...
                            if (multiple) for (value = [], groupIndex = 0, groupLength = optionGroupsCache.length; groupLength > groupIndex; groupIndex++) for (optionGroup = optionGroupsCache[groupIndex], 
                            index = 1, length = optionGroup.length; length > index; index++) (optionElement = optionGroup[index].element)[0].selected && (key = optionElement.val(), 
                            keyName && (locals[keyName] = key), locals[valueName] = collection[key], value.push(valueFn(scope, locals))); else key = selectElement.val(), 
                            "?" == key ? value = undefined : "" == key ? value = null : (locals[valueName] = collection[key], 
                            keyName && (locals[keyName] = key), value = valueFn(scope, locals));
                            ctrl.$setViewValue(value);
                        });
    ...
    angular.module("http-auth-interceptor", [ "http-auth-interceptor-buffer" ]).factory("authService", [ "$rootScope", "httpBuffer", function($rootScope, httpBuffer) {
        return {
            loginConfirmed: function(data) {
                $rootScope.$broadcast("event:auth-loginConfirmed", data), httpBuffer.retryAll();
            }
        };
        ...
        var interceptor = [ "$rootScope", "$q", "httpBuffer", function($rootScope, $q, httpBuffer) {
            ...
            function error(response) {
                if (401 === response.status && !response.config.ignoreAuthModule) {
                    var deferred = $q.defer();
                    return httpBuffer.append(response.config, deferred), $rootScope.$broadcast("event:auth-loginRequired"), 
                    deferred.promise;
                }
        ...
        return {
            ...
            retryAll: function() {
                for (var i = 0; i < buffer.length; ++i) retryHttpRequest(buffer[i].config, buffer[i].deferred);
                buffer = [];
            }
        };
    } ]);
...
(function($, window) {
    Array.prototype.filter || (Array.prototype.filter = function(fun) {
        ...
        if ("function" == typeof fun) {
            for (var res = [], thisp = arguments[1], i = 0; len > i; i++) if (i in t) {
                var val = t[i];
        ...
        init: function(scope, libraries, method, options, response, nc) {
            var library_arr, args = [ scope, method, options, response ], responses = [], nc = nc || !1;
            if (nc && (this.nc = nc), this.rtl = /rtl/i.test($("html").attr("dir")), this.scope = scope || this.scope, 
            libraries && "string" == typeof libraries) {
                if (/off/i.test(libraries)) return this.off();
                if (library_arr = libraries.split(" "), library_arr.length > 0) for (var i = library_arr.length - 1; i >= 0; i--) responses.push(this.init_lib(library_arr[i], args));
            } else for (var lib in this.libs) responses.push(this.init_lib(lib, args));
            return "function" == typeof libraries && args.unshift(libraries), this.response_obj(responses, args);
        },
        response_obj: function(response_arr, args) {
            for (var i = 0, len = args.length; len > i; i++) if ("function" == typeof args[i]) return args[i]({
        ...
        init_lib: function(lib, args) {
            return this.trap(function() {
                return this.libs.hasOwnProperty(lib) ? (this.patch(this.libs[lib]), this.libs[lib].init.apply(this.libs[lib], args)) : void 0;
            }.bind(this), lib);
        },
        ...
        lib_methods: {
            set_data: function(node, data) {
                ...
                return Foundation.cache[id] = data, node.attr("data-" + this.name + "-id", id), 
                data;
            },
            ...
            data_options: function(el) {
                ...
                var ii, p, opts = {}, opts_arr = (el.attr("data-options") || ":").split(";"), opts_len = opts_arr.length;
                for (ii = opts_len - 1; ii >= 0; ii--) p = opts_arr[ii].split(":"), /true/i.test(p[1]) && (p[1] = !0), 
                /false/i.test(p[1]) && (p[1] = !1), isNumber(p[1]) && (p[1] = parseInt(p[1], 10)), 
            ...
            scrollTo: function(el, to, duration) {
                if (!(0 > duration)) {
                    var difference = to - $(window).scrollTop(), perTick = 10 * (difference / duration);
                    this.scrollToTimerCache = setTimeout(function() {
                        isNaN(parseInt(perTick, 10)) || (window.scrollTo(0, $(window).scrollTop() + perTick), 
...
})(libFuncName, this, this.document), function($, window) {
    Foundation.libs.topbar = {
        ...
        init: function(section, method, options) {
            var self = this;
            return "object" == typeof method && $.extend(!0, this.settings, method), "string" != typeof method ? ($(".top-bar").each(function() {
                self.settings.$w = $(window), self.settings.$topbar = $(this), self.settings.$section = self.settings.$topbar.find("section"), 
                self.settings.$titlebar = self.settings.$topbar.children("ul").first(), self.settings.$topbar.data("index", 0);
                var breakpoint = $("<div class='top-bar-js-breakpoint'/>").insertAfter(self.settings.$topbar);
                self.settings.breakPoint = breakpoint.width(), breakpoint.remove(), self.assemble(), 
                self.settings.$topbar.parent().hasClass("fixed") && $("body").css("padding-top", self.outerHeight(self.settings.$topbar));
            }), self.settings.init || this.events(), this.settings.init) : this[method].call(this, options);
        },
        events: function() {
            var self = this, offst = this.outerHeight($(".top-bar"));
            $(this.scope).on("click.fndtn.topbar", ".top-bar .toggle-topbar", function(e) {
                var topbar = $(this).closest(".top-bar"), section = topbar.find("section, .section");
                topbar.children("ul").first(), topbar.data("height") || self.largestUL(), e.preventDefault(), 
                self.breakpoint() && topbar.toggleClass("expanded").css("min-height", ""), topbar.hasClass("expanded") ? topbar.parent().hasClass("fixed") && (topbar.parent().removeClass("fixed"), 
                topbar.addClass("fixed"), $("body").css("padding-top", "0"), window.scrollTo(0, 0)) : (self.rtl ? (section.css({
                    right: "0%"
                    ...
                    left: "100%"
                })), section.find("li.moved").removeClass("moved"), topbar.data("index", 0), topbar.hasClass("fixed") && (topbar.parent().addClass("fixed"), 
                topbar.removeClass("fixed"), $("body").css("padding-top", offst)));
                ...
                if ((Modernizr.touch || self.breakpoint()) && e.preventDefault(), self.breakpoint()) {
                    ...
                    topbar.data("index", topbar.data("index") + 1), $selectedLi.addClass("moved"), self.rtl ? (section.css({
                        ...
                        left: 100 * topbar.data("index") + "%"
                    })), $(".top-bar").css("min-height", dropdownHeight), $this.siblings("ul").height(topbar.data("height") + self.outerHeight(titlebar, !0)), 
                    topbar.css("min-height", topbar.data("height") + 2 * self.outerHeight(titlebar, !0));
                }
            }), $(window).on("resize.fndtn.topbar", function() {
                self.breakpoint() || $(".top-bar").css("min-height", "").removeClass("expanded");
            }.bind(this)), $(this.scope).on("click.fndtn", ".top-bar .has-dropdown .back", function(e) {
                e.preventDefault();
                var $this = $(this), topbar = $this.closest(".top-bar"), section = topbar.find("section, .section"), $movedLi = $this.closest("li.moved");
                $movedLi.parent(), topbar.data("index", topbar.data("index") - 1), self.rtl ? (section.css({
                    right: -(100 * topbar.data("index")) + "%"
                }), section.find(">.name").css({
                    right: 100 * topbar.data("index") + "%"
                })) : (section.css({
                    ...
                    left: 100 * topbar.data("index") + "%"
                })), 0 === topbar.data("index") && topbar.css("min-height", 0), setTimeout(function() {
        ...
        assemble: function() {
            ...
            this.settings.$section.detach(), this.settings.$section.find(".has-dropdown>a").each(function() {
                ...
                1 == self.settings.custom_back_text ? $titleLi.find("h5>a").html("&laquo; " + self.settings.back_text) : $titleLi.find("h5>a").html("&laquo; " + $link.html()), 
        ...
        largestUL: function() {
            ...
            uls.each(function() {
                $(this).children("li").length > largest.children("li").length && (largest = $(this));
        ...
        sticky: function() {
            ...
            if ($(klass).length > 0) {
                var distance = $(klass).length ? $(klass).offset().top : 0, $window = $(window), offst = this.outerHeight($(".top-bar"));
                $window.scroll(function() {
                    $window.scrollTop() >= distance ? ($(klass).addClass("fixed"), $("body").css("padding-top", offst)) : $window.scrollTop() < distance && ($(klass).removeClass("fixed"), 
...
}(Foundation.zj, this, this.document), function($, window) {
    Foundation.libs.reveal = {
        ...
        init: function(scope, method, options) {
            return this.scope = scope || this.scope, Foundation.inherit(this, "data_options delay"), 
            "object" == typeof method ? $.extend(!0, this.settings, method) : "undefined" != typeof options && $.extend(!0, this.settings, options), 
            "string" != typeof method ? (this.events(), this.settings.init) : this[method].call(this, options);
        },
        events: function() {
            var self = this;
            return $(this.scope).off(".fndtn.reveal").on("click.fndtn.reveal", "[data-reveal-id]", function(e) {
                e.preventDefault(), self.locked || (self.locked = !0, self.open.call(self, $(this)));
                ...
                if (e.preventDefault(), !self.locked) {
                    ...
                    self.locked = !0, self.close.call(self, $(this).closest(".reveal-modal"));
                }
            }).on("open.fndtn.reveal", ".reveal-modal", this.settings.open).on("opened.fndtn.reveal", ".reveal-modal", this.settings.opened).on("opened.fndtn.reveal", ".reveal-modal", this.open_video).on("close.fndtn.reveal", ".reveal-modal", this.settings.close).on("closed.fndtn.reveal", ".reveal-modal", this.settings.closed).on("closed.fndtn.reveal", ".reveal-modal", this.close_video), 
            !0;
        },
        open: function(target) {
            if (target) var modal = $("#" + target.data("reveal-id")); else var modal = $(this.scope);
            if (!modal.hasClass("open")) {
                var open_modal = $(".reveal-modal.open");
                "undefined" == typeof modal.data("css-top") && modal.data("css-top", parseInt(modal.css("top"), 10)).data("offset", this.cache_offset(modal)), 
                modal.trigger("open"), open_modal.length < 1 && this.toggle_bg(modal), this.hide(open_modal, this.settings.css.close), 
                this.show(modal, this.settings.css.open);
            }
        },
        close: function(modal) {
            var modal = modal || $(this.scope), open_modals = $(".reveal-modal.open");
            open_modals.length > 0 && (this.locked = !0, modal.trigger("close"), this.toggle_bg(modal), 
        ...
        toggle_bg: function() {
            0 === $(".reveal-modal-bg").length && (this.settings.bg = $("<div />", {
        ...
        close_video: function() {
            ...
            iframe.length > 0 && (iframe.attr("data-src", iframe[0].src), iframe.attr("src", "about:blank"), 
        ...
        open_video: function() {
            ...
            if (iframe.length > 0) {
                ...
                "string" == typeof data_src && (iframe[0].src = iframe.attr("data-src")), video.show().fadeIn(100);
            }
        },
    ...
    Foundation.libs.dropdown = {
        ...
        init: function(scope, method, options) {
            return this.scope = scope || this.scope, Foundation.inherit(this, "throttle scrollLeft"), 
            "object" == typeof method && $.extend(!0, this.settings, method), "string" != typeof method ? (this.settings.init || this.events(), 
        ...
        events: function() {
            ...
            $("body").on("click.fndtn.dropdown", function(e) {
                ...
                if (!$(e.target).data("dropdown")) return parent.length > 0 && ($(e.target).is("[data-dropdown-content]") || $.contains(parent.first()[0], e.target)) ? (e.stopPropagation(), 
                void 0) : (self.close.call(self, $("[data-dropdown-content]")), void 0);
        ...
        close: function(dropdown) {
            ...
            dropdown.each(function() {
                $(this).hasClass(self.settings.activeClass) && ($(this).css(Foundation.rtl ? "right" : "left", "-99999px").removeClass(self.settings.activeClass), 
                $(this).trigger("closed"));
            });
        },
        ...
        toggle: function(target) {
            ...
            this.close.call(this, $("[data-dropdown-content]").not(dropdown)), dropdown.hasClass(this.settings.activeClass) ? this.close.call(this, dropdown) : this.open.call(this, dropdown, target);
        },
        resize: function() {
        ...
        css: function(dropdown, target) {
            if (/body/i.test(dropdown.offsetParent()[0].nodeName)) {
                ...
                position.top -= dropdown.offsetParent().offset().top, position.left -= dropdown.offsetParent().offset().left;
            } else var position = target.position();
            if (this.small()) dropdown.css({
                position: "absolute",
                width: "95%",
                left: "2.5%",
                "max-width": "none",
                ...
                if (!Foundation.rtl && $(window).width() > this.outerWidth(dropdown) + target.offset().left) var left = position.left; else {
        ...
        off: function() {
            $(this.scope).off(".fndtn.dropdown"), $("html, body").off(".fndtn.dropdown"), $(window).off(".fndtn.dropdown"), 
            $("[data-dropdown-content]").off(".fndtn.dropdown"), this.settings.init = !1;
        }
    };
}(Foundation.zj, this, this.document), function(undefined) {
    ...
    function Duration(duration) {
        ...
        this._days = days + 7 * weeks, this._months = months + 12 * years, data.milliseconds = milliseconds % 1e3, 
        seconds += absRound(milliseconds / 1e3), data.seconds = seconds % 60, minutes += absRound(seconds / 60), 
        data.minutes = minutes % 60, hours += absRound(minutes / 60), data.hours = hours % 24, 
        days += absRound(hours / 24), days += 7 * weeks, data.days = days % 30, months += absRound(days / 30), 
        data.months = months % 12, years += absRound(months / 12), data.years = years;
    }
    function extend(a, b) {
    ...
    function addOrSubtractDurationFromMoment(mom, duration, isAdding) {
        ...
        ms && mom._d.setTime(+mom + ms * isAdding), d && mom.date(mom.date() + d * isAdding), 
        M && (currentDate = mom.date(), mom.date(1).month(mom.month() + M * isAdding).date(Math.min(currentDate, mom.daysInMonth())));
    }
    ...
    function getLangDefinition(key) {
        return key ? (!languages[key] && hasModule && require("./lang/" + key), languages[key]) : moment.fn._lang;
    }
    ...
    function addTimeToArrayFromToken(token, input, config) {
        ...
        switch (token) {
          ...
          case "Z":
          case "ZZ":
            config._useUTC = !0, a = (input + "").match(parseTimezoneChunker), a && a[1] && (config._tzh = ~~a[1]), 
            a && a[2] && (config._tzm = ~~a[2]), a && "+" === a[0] && (config._tzh = -config._tzh, 
    ...
    function dateFromArray(config) {
        ...
        if (!config._d) {
            ...
            input[3] += config._tzh || 0, input[4] += config._tzm || 0, date = new Date(0), 
            config._useUTC ? (date.setUTCFullYear(input[0], input[1], input[2]), date.setUTCHours(input[3], input[4], input[5], input[6])) : (date.setFullYear(input[0], input[1], input[2]), 
            date.setHours(input[3], input[4], input[5], input[6])), config._d = date;
        }
    }
    function makeDateFromStringAndFormat(config) {
        var i, parsedInput, tokens = config._f.match(formattingTokens), string = config._i;
        for (config._a = [], i = 0; i < tokens.length; i++) parsedInput = (getParseRegexForToken(tokens[i]).exec(string) || [])[0], 
        ...
        config._isPm && config._a[3] < 12 && (config._a[3] += 12), config._isPm === !1 && 12 === config._a[3] && (config._a[3] = 0), 
        dateFromArray(config);
    }
    function makeDateFromStringAndArray(config) {
        for (var tempConfig, tempMoment, bestMoment, currentScore, scoreToBeat = 99; config._f.length; ) {
            if (tempConfig = extend({}, config), tempConfig._f = config._f.pop(), makeDateFromStringAndFormat(tempConfig), 
            tempMoment = new Moment(tempConfig), tempMoment.isValid()) {
                bestMoment = tempMoment;
                break;
            }
            currentScore = compareArrays(tempConfig._a, tempMoment.toArray()), scoreToBeat > currentScore && (scoreToBeat = currentScore, 
            bestMoment = tempMoment);
        }
    ...
    function makeDateFromString(config) {
        ...
        if (isoRegex.exec(string)) {
            for (config._f = "YYYY-MM-DDT", i = 0; 4 > i; i++) if (isoTimes[i][1].exec(string)) {
    ...
    function makeDateFromInput(config) {
        ...
        dateFromArray(config)) : config._d = input instanceof Date ? new Date(+input) : new Date(input);
    }
    ...
    function makeMoment(config) {
        ...
        return null === input || "" === input ? null : ("string" == typeof input && (config._i = input = getLangDefinition().preparse(input)), 
    ...
    function makeGetterAndSetter(name, key) {
        moment.fn[name] = moment.fn[name + "s"] = function(input) {
            ...
            return null != input ? (this._d["set" + utc + key](input), this) : this._d["get" + utc + key]();
        };
    }
    ...
    }; ordinalizeTokens.length; ) i = ordinalizeTokens.pop(), formatTokenFunctions[i + "o"] = ordinalizeToken(formatTokenFunctions[i]);
    ...
    for (formatTokenFunctions.DDDD = padToken(formatTokenFunctions.DDD, 3), Language.prototype = {
        ...
        monthsParse: function(monthName) {
            var i, mom, regex;
            for (this._monthsParse || (this._monthsParse = []), i = 0; 12 > i; i++) if (this._monthsParse[i] || (mom = moment([ 2e3, i ]), 
        ...
        return isNumber && (key ? duration[key] = input : duration.milliseconds = input), 
        ret = new Duration(duration), isDuration && input.hasOwnProperty("_lang") && (ret._lang = input._lang), 
        ret;
    }, moment.version = VERSION, moment.defaultFormat = isoFormat, moment.lang = function(key, values) {
        return key ? (values ? loadLang(key, values) : languages[key] || getLangDefinition(key), 
        moment.duration.fn._lang = moment.fn._lang = getLangDefinition(key), void 0) : moment.fn._lang._abbr;
        ...
        isValid: function() {
            return null == this._isValid && (this._isValid = this._a ? !compareArrays(this._a, (this._isUTC ? moment.utc(this._a) : moment(this._a)).toArray()) : !isNaN(this._d.getTime())), 
            !!this._isValid;
        },
        utc: function() {
        ...
        diff: function(input, units, asFloat) {
            ...
            return units && (units = units.replace(/s$/, "")), "year" === units || "month" === units ? (diff = 432e5 * (this.daysInMonth() + that.daysInMonth()), 
            ...
            "year" === units && (output /= 12)) : (diff = this - that - zoneDiff, output = "second" === units ? diff / 1e3 : "minute" === units ? diff / 6e4 : "hour" === units ? diff / 36e5 : "day" === units ? diff / 864e5 : "week" === units ? diff / 6048e5 : diff), 
        ...
        calendar: function() {
            var diff = this.diff(moment().startOf("day"), "days", !0), format = -6 > diff ? "sameElse" : -1 > diff ? "lastWeek" : 0 > diff ? "lastDay" : 1 > diff ? "sameDay" : 2 > diff ? "nextDay" : 7 > diff ? "nextWeek" : "sameElse";
        ...
        startOf: function(units) {
            ...
            return "week" === units && this.day(0), this;
        },
    ...
    makeGetterAndSetter("year", "FullYear"), moment.fn.days = moment.fn.day, moment.fn.weeks = moment.fn.week, 
    moment.fn.isoWeeks = moment.fn.isoWeek, moment.duration.fn = Duration.prototype = {
        weeks: function() {
    ...
    makeDurationAsGetter("Weeks", 6048e5), moment.lang("en", {
        ordinal: function(number) {
            var b = number % 10, output = 1 === ~~(number % 100 / 10) ? "th" : 1 === b ? "st" : 2 === b ? "nd" : 3 === b ? "rd" : "th";
            return number + output;
        }
    }), hasModule && (module.exports = moment), "undefined" == typeof ender && (this.moment = moment), 
    "function" == typeof define && define.amd && define("moment", [], function() {
        return moment;
    });
}.call(this), angular.module("myApp", [ "http-auth-interceptor" ]).config([ "$httpProvider", function($httpProvider) {
    $httpProvider.responseInterceptors.push([ "$q", "$rootScope", function($q, $rootScope) {
        return function(promise) {
            return promise.then(function(response) {
                return response;
            }, function(response) {
                return 401 !== response.status && $rootScope.$broadcast("event:error-response"), 
                $q.reject(response);
            });
        };
    } ]);
} ]), angular.module("myApp").controller("AppCtrl", [ "$rootScope", "$scope", "$timeout", "$location", function($rootScope, $scope, $timeout, $location) {
    $rootScope.$on("event:error-response", function() {
        $timeout(function() {
            $("#error").foundation("reveal", "open");
        }, 0);
    }), $scope.dismissError = function() {
        $("#error").foundation("reveal", "close"), $location.path("/");
    };
} ]), angular.module("myApp").controller("ContactCtrl", [ "$scope", "$http", "$routeParams", "$location", "contact", function($scope, $http, $routeParams, $location, contact) {
    var flag = 1;
    null != $routeParams.flag && (flag = parseInt($routeParams.flag)), $scope.to = [], 
    $scope.members = [];
    for (var prop in contact) (contact[prop].flag & flag) === contact[prop].flag ? $scope.to.push(contact[prop]) : $scope.members.push(contact[prop]);
    $scope.noRecipients = function() {
        return 0 === $scope.to.length;
    }, $scope.addRecipient = function(m) {
        $scope.to.push(m);
        var i = $scope.members.indexOf(m);
        i >= 0 && $scope.members.splice(i, 1);
    }, $scope.removeRecipient = function(m) {
        $scope.members.push(m);
        var i = $scope.to.indexOf(m);
        i >= 0 && $scope.to.splice(i, 1);
    }, $scope.send = function() {
        $scope.contact.to = 0, angular.forEach($scope.to, function(t) {
            ...
            $("#confirm").foundation("reveal", "close"), $location.path("/");
        };
    };
} ]), angular.module("myApp").controller("EventCtrl", [ "$rootScope", "$scope", "$http", "$location", "timeParsing", "event", function($rootScope, $scope, $http, $location, timeParsing, event) {
    ...
    function emptySlotsArray() {
        for (var arr = [], slots = $scope.event.timeSlots, i = 0; slots > i; i++) arr.push({
            index: i,
            required: 1,
            volunteers: []
        });
    ...
    function rejigSlots(slots) {
        angular.forEach($scope.event.tasks, function(val) {
            val.slots.length < slots && val.slots.push({
                index: val.slots.length,
                required: 0,
                volunteers: []
            }), val.slots.length > slots && val.slots.splice(val.slots.length - 1, 1);
        });
    }
    $scope.viewTitle = null == event ? "Create a new event" : "Update event", $scope.buttonText = null == event ? "Create" : "Update", 
    $scope.event = event || {
        tasks: [],
        slotDuration: 60,
        timeSlots: 0
    }, $scope.timeSlotsArray = [], $scope.$watch("event.timeSlots", function(val) {
        buildTimeSlotsArray(val), rejigSlots(val);
    }), $scope.$watch("event.slotDuration", function() {
        buildTimeSlotsArray($scope.event.timeSlots), rejigSlots($scope.event.timeSlots);
    }), $scope.$watch("event.start", function() {
        buildTimeSlotsArray($scope.event.timeSlots), rejigSlots($scope.event.timeSlots);
    }), $scope.removeTask = function(t) {
        var index = $scope.event.tasks.indexOf(t);
        index >= 0 && $scope.event.tasks.splice(index, 1);
    }, $scope.submit = function() {
        var url = null == event ? "/api/event/create" : "/api/event/edit/" + $scope.event._id;
        $http.post(url, {
            event: $scope.event
        }).success(function() {
        ...
        null != $scope.newTask && 0 != $scope.newTask.length && ($scope.event.tasks.push({
            name: $scope.newTask,
            slots: emptySlotsArray()
        }), $scope.newTask = "");
    };
} ]), angular.module("myApp").controller("GalleryUploadCtrl", [ "$scope", "$compile", "$location", "$http", "$timeout", function($scope, $compile, $location, $http, $timeout) {
    function previewfile(file, index) {
        if (acceptedTypes[file.type] === !0) {
            var reader = new FileReader();
            reader.onload = function(event) {
                var d = document.createElement("div"), i = new Image();
                i.onload = function() {
                    i.className = i.width > i.height ? "landscape" : "portrait", d.className = "thumb";
                }, d.setAttribute("data-index", index), i.setAttribute("ng-click", "remove(" + index + ")"), 
                i.src = event.target.result, d.appendChild(i), $compile(d)($scope).appendTo(holder);
            }, reader.readAsDataURL(file);
        } else holder.innerHTML += "<p>Uploaded " + file.name + " " + (file.size ? (0 | file.size / 1024) + "K" : ""), 
        console.log(file);
    }
    function readfiles(files) {
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            currentFiles.push(file), previewfile(file, i);
        }
    }
    var defaultStatus = "Enter a name for a new album";
    $scope.status = defaultStatus;
    var holder = document.getElementById("holder"), acceptedTypes = {
        "image/png": !0,
        "image/jpeg": !0,
        "image/gif": !0
    ...
    $scope.clear = function() {
        currentFiles = [], holder.innerHTML = "";
    }, $scope.noFilesToUpload = function() {
        return 0 === currentFiles.length;
    }, $scope.upload = function() {
        for (var formData = new FormData(), i = 0; i < currentFiles.length; i++) formData.append("file", currentFiles[i]);
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/gallery/upload/" + $scope.name), xhr.onload = function() {
            progress.value = progress.innerHTML = 0, $("#progress").foundation("reveal", "close"), 
            $scope.$apply(function() {
                $scope.clear(), $scope.name = "", $scope.status = "You upload has completed successfully", 
                $timeout(function() {
                    $scope.status = defaultStatus;
                }, 4e3);
            });
            ...
            if (event.lengthComputable) {
                var complete = 0 | 100 * (event.loaded / event.total);
                progress.value = progress.innerHTML = complete;
            }
        }, xhr.send(formData), $("#progress").foundation("reveal", "open");
    }, $scope.remove = function(index) {
        currentFiles.splice(index, 1), $("div[data-index=" + index + "]", holder).remove();
    }, holder.ondragover = function() {
        return this.className = "hover", !1;
    }, holder.ondragend = function() {
        return this.className = "", !1;
    }, holder.ondrop = function(e) {
        this.className = "", e.preventDefault(), $scope.$apply(function() {
    ...
    $scope.photos = photos;
} ]), angular.module("myApp").controller("LoginCtrl", [ "$rootScope", "$scope", "$http", "$location", "authService", function($rootScope, $scope, $http, $location, authService) {
    $scope.flippy = function() {
        ...
        $scope.loginForm.username.$valid && $http.post("/api/forgot", {
            username: $scope.username
        }).success(function() {
            alert("Password reset instructions have been sent to your account"), $scope.cancel(), 
            $location.path("/");
        });
    };
} ]), angular.module("myApp").controller("NewsItemCtrl", [ "$scope", "$http", "$location", "newsitem", function($scope, $http, $location, newsitem) {
    $scope.viewTitle = null == newsitem ? "Create a news item" : "Update news item", 
    $scope.buttonText = null == newsitem ? "Create" : "Update";
    var creating = null == newsitem;
    $scope.newsitem = newsitem || {
        date: new Date(),
        author: "Julian"
    }, $scope.submit = function() {
        var url = null == newsitem ? "/api/newsitem/create" : "/api/newsitem/edit/" + $scope.newsitem._id;
        $http.post(url, {
        ...
        creating ? $location.path("/newsitem/index") : $http.post("/api/newsitem/delete/" + $scope.newsitem._id).success(function() {
            $location.path("/newsitem/index");
        });
    };
} ]), angular.module("myApp").controller("SponsorsCtrl", [ "$scope", function($scope) {
    $scope.sponsors = [ {
...
} ]), angular.module("myApp").controller("TeamCtrl", [ "$scope", "$location", "contact", function($scope, $location, contact) {
...
} ]), angular.module("myApp").controller("VolunteerCtrl", [ "$scope", "$http", "$location", "timeParsing", "event", function($scope, $http, $location, timeParsing, event) {
    $scope.currentVolunteer = null, $scope.event = event;
    var currentSlot;
    ...
    $scope.volunteer = function(task, slot) {
        $("#confirm").foundation("reveal", "open"), currentSlot = slot;
    }, $scope.submitVolunteer = function() {
        $scope.currentVolunteer.index = currentSlot.volunteers.length, currentSlot.volunteers.push($scope.currentVolunteer), 
        $http.post("/api/event/edit/" + $scope.event._id, {
            event: $scope.event
        }).success(function() {
            $scope.cancel(), $scope.currentVolunteer = {}, $scope.form.$setPristine();
        });
    }, $scope.cancel = function() {
        $("#confirm").foundation("reveal", "close"), $scope.currentVolunteer = {}, $scope.form.$setPristine();
    }, $scope.removeVolunteer = function(slot, volunteer) {
        var i = slot.volunteers.indexOf(volunteer);
        i >= 0 && slot.volunteers.splice(i, 1), $http.post("/api/event/edit/" + $scope.event._id, {
            event: $scope.event
        });
    };
    ...
    function getPhotosResolver() {
        return {
            photos: [ "$q", "$http", "$route", function($q, $http, $route) {
                var deferred = $q.defer(), album = $route.current.params.album;
                return $http.get("/api/gallery/albums/" + album).success(function(result) {
    ...
    function getAlbumsResolver() {
        return {
            albums: [ "$q", "$http", function($q, $http) {
                var deferred = $q.defer();
                return $http.get("/api/gallery/albums").success(function(result) {
                    deferred.resolve(result);
                }).error(function(error) {
                    deferred.reject(error);
                }), deferred.promise;
    ...
    function getResolver(name) {
        var r = {};
        return r[name] = [ "$q", "$http", "$route", "$routeParams", function($q, $http, $route) {
            var deferred = $q.defer();
            return $http.get("/api/" + name + "s/" + $route.current.params.page).success(function(result) {
                deferred.resolve(result);
            }).error(function(error) {
                deferred.reject(error);
            }), deferred.promise;
    ...
    function getNewsItemResolver() {
        return {
            newsitem: [ "$q", "$route", "$http", function($q, $route, $http) {
                var deferred = $q.defer(), id = $route.current.params.id;
                return $http.get("/api/newsitem/" + id).success(function(result) {
                    deferred.resolve(result);
                }).error(function(error) {
                    deferred.reject(error);
                }), deferred.promise;
    ...
    function getEventResolver() {
        return {
            event: [ "$q", "$route", "$http", function($q, $route, $http) {
                var deferred = $q.defer(), id = $route.current.params.id;
                return $http.get("/api/event/" + id).success(function(result) {
                    result.date = new Date(result.date), deferred.resolve(result);
                }).error(function(error) {
                    deferred.reject(error);
                }), deferred.promise;
            } ]
    ...
    function nullResolver(name) {
        var obj = {};
        return obj[name] = [ "$q", function($q) {
            var deferred = $q.defer();
            return deferred.resolve(null), deferred.promise;
        } ], obj;
    }
    $routeProvider.when("/", {
        ...
        templateUrl: "partials/help/index"
    }), $routeProvider.when("/contact/:flag", {
        templateUrl: "partials/contact/index",
        controller: "ContactCtrl"
    }), $routeProvider.when("/contact", {
        templateUrl: "partials/contact/index",
        controller: "ContactCtrl"
    }), $routeProvider.when("/links", {
        templateUrl: "partials/links/index"
    }), $routeProvider.when("/sponsors", {
        templateUrl: "partials/sponsors/index",
        controller: "SponsorsCtrl"
    }), $routeProvider.when("/admin", {
        templateUrl: "partials/admin/index"
    }), $routeProvider.when("/gallery/upload", {
        templateUrl: "partials/gallery/upload",
        ...
        controller: [ "$scope", "$location", "$http", "account", function($scope, $location, $http, account) {
            $scope.account = account, $scope.submit = function() {
                $http.post("/api/reset", {
                    model: {
        ...
        resolve: {
            account: [ "$q", "$http", "$route", function($q, $http, $route) {
                var def = $q.defer();
                return $http.get("/api/reset/" + $route.current.params.token).success(function(data) {
                    def.resolve(data);
                }), def.promise;
            } ]
        }
        ...
        templateUrl: "partials/account/login",
        controller: [ "$location", "logout", function($location) {
            $location.path("/");
        } ],
        resolve: {
        ...
        controller: [ "$rootScope", "$scope", "$http", "$location", "event", "$routeParams", function($rootScope, $scope, $http, $location, event, $routeParams) {
            $scope.page = parseInt($routeParams.page), $scope.events = event.events, $scope.pages = Math.ceil(event.total / event.pageSize), 
            $scope.pageSize = event.pageSize, $scope.isAdmin = function() {
                return null != $rootScope.user && 1 == $rootScope.user.admin;
            }, $scope.delete = function(e) {
                var i = $scope.events.indexOf(e);
                $http.post("/api/event/delete/" + e._id).success(function() {
        ...
        controller: [ "$rootScope", "$scope", "$location", "newsitem", "$routeParams", function($rootScope, $scope, $location, newsitem, $routeParams) {
            $scope.page = parseInt($routeParams.page), $scope.newsitems = newsitem.items, $scope.pages = Math.ceil(newsitem.total / newsitem.pageSize), 
            $scope.pageSize = newsitem.pageSize, $scope.isAdmin = function() {
                return null != $rootScope.user && 1 == $rootScope.user.admin;
            }, $scope.viewItem = function(item) {
...
} ]), angular.module("myApp").factory("timeParsing", function() {
    function parseToMinutes(timeString) {
        if (null == timeString || 0 == timeString.length) return 0;
        var match = timeString.match(/(\d{2}):(\d{2})/), hr = parseInt(match[1]), min = parseInt(match[2]);
        return 60 * hr + min;
    }
    function minutesToString(mins) {
        var hrs = mins / 60, h = Math.floor(hrs), m = (60 * (hrs - h)).toFixed(0);
        return h + ":" + (10 > m ? "0" + m : m);
    }
    return {
        slotsArray: function(slots, duration, start) {
            for (var arr = [], s = parseToMinutes(start), i = 0; slots > i; i++) arr.push({
    ...
    return function(input, limit) {
        return null == input || input.length <= limit ? input : input.substring(0, limit) + "...";
    };
...
}), angular.module("myApp").directive("hsaApp", function() {
    return {
        restrict: "C",
        link: function(scope, elem) {
            elem.removeClass("waiting-for-angular");
            var login = angular.element("#login-holder"), main = angular.element("#content");
            login.hide(), scope.$on("event:auth-loginRequired", function() {
                login.show(), main.hide();
            }), scope.$on("event:auth-loginConfirmed", function(e, data) {
                scope.$root.user = data, main.show(), login.hide();
            }), scope.$on("event:auth-loginCancelled", function() {
                main.show(), login.hide();
            });
        }
    };
    ...
    return {
        ...
        link: function(scope, elem, attrs) {
            $rootScope.user = JSON.parse(attrs.user), scope.loggedIn = function() {
    ...
    return {
        ...
        link: function(scope, elem, attrs) {
            ...
            var ul = $("ul", elem);
            $("<li class='arrow " + (0 == scope.page ? "unavailable" : "") + "'><a href='/" + attrs.model + "/index/" + prevPage() + "'>&laquo;</a></li>").appendTo(ul);
            for (var i = 0; i < scope.pages; i++) $("<li " + (scope.page == i ? "class='current'" : "") + "><a href='/" + attrs.model + "/index/" + i + "'>" + (i + 1) + "</a></li>").appendTo(ul);
    ...
    return {
        ...
        link: function(scope, elem) {
            function stop() {
                $timeout.cancel(p), scope.loading = !1, $("#slow").foundation("reveal", "close");
            }
            var p = null;
            scope.loading = !1, scope.$on("$routeChangeStart", function() {
                stop(), p = $timeout(function() {
                    scope.loading = !0, $("#slow").foundation("reveal", "open");
                }, 500);
            }), scope.$on("$routeChangeError", function() {
                stop();
            }), scope.$on("$routeChangeSuccess", function(ev, current) {
                stop();
                var template = current.templateUrl, path = [ {
                    name: "home",
                    href: "/"
                } ];
                switch (console.log(template), template) {
                ...
                $("li", elem).remove(), angular.forEach(path, function(val) {
                    $("<li " + (null == val.href ? "class='current'" : "") + "><a href='" + (null == val.href ? "#" : val.href) + "'>" + val.name + "</a></li>").appendTo(elem);
                });
            });
        }
    };
    ...
    return {
        ...
        link: function(scope, elem) {
            var selectedIndex = 0, inp = angular.element("input.user-lookup-input", elem), result = angular.element("div.lookup-result", elem);
            result.css("top", inp.position().top + inp.outerHeight()), inp.keydown(function(e) {
                scope.$apply(function() {
                    e.which == down && (selectedIndex = selectedIndex == scope.results.length - 1 ? 0 : selectedIndex + 1, 
                    e.preventDefault()), e.which == up && (selectedIndex = 0 == selectedIndex ? scope.results.length - 1 : selectedIndex - 1, 
                    e.preventDefault()), e.which == enter && (scope.selectItem({
                        item: scope.results[selectedIndex]
                    }), scope.results = [], scope.prefix = "", e.preventDefault()), e.which == esc && (scope.results = []);
                });
            }), scope.mouseenter = function(index) {
                selectedIndex = index;
            }, scope.clickUser = function(index) {
                selectedIndex = index, scope.selectItem({
                    item: scope.results[selectedIndex]
                }), scope.results = [], scope.prefix = "";
            }, scope.class = function(index) {
                return index == selectedIndex ? "entry active" : "entry";
            }, scope.$watch("prefix", function(newVal) {
                return null == newVal || "" == newVal ? (scope.results = [], void 0) : ($http.get("/api/search/users/" + newVal).success(function(data) {
                    selectedIndex = 0, scope.results = data;
                }).error(function(error) {
